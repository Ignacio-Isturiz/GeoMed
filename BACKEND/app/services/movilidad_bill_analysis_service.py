...
_VELOCIDAD_INTENSIDAD_PATH = os.path.join(_DATASETS_DIR, "velocidad_e_intensidad_vehicular_en_medellin_clean.csv")
_PASAJEROS_MOVILIZADOS_PATH = os.path.join(_DATASETS_DIR, "Pasajeros_movilizados_clean.csv")
_SIMMTRAFFIC_PATH = os.path.join(_DATASETS_DIR, "simmtrafficdata_clean.csv")
...
def _load_latest_tarifas_energia(n: int = 30) -> str:
    ...
    lines = []
    try:
        with open(_VELOCIDAD_INTENSIDAD_PATH, encoding="utf-8-sig") as f:
...
def _load_latest_tarifas_acueducto(n: int = 30) -> str:
    ...
    lines = []
    try:
        with open(_PASAJEROS_MOVILIZADOS_PATH, encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
