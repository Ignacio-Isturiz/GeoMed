...
class DatasetsService:
    ...
    def _get_proyecciones_poblacion_file(self) -> Path:
        return self.datasets_path / "proyecciones_de_poblacion_medellin_2019_clean.csv"

    def _get_pasajeros_movilizados_file(self) -> Path:
        return self.datasets_path / "Pasajeros_movilizados_clean.csv"

    def _get_aforos_vehiculares_file(self) -> Path:
        return self.datasets_path / "Aforos_Vehiculares_clean.csv"

    @staticmethod
    def _normalize_movilidad_dataset(dataset: str) -> str:
        normalized = (dataset or "").strip().lower().replace("_", "-")
        aliases = {
            "gas": "gas",
            "aforos-vehiculares-clean": "gas",
            "aforos-vehiculares-clean.csv": "gas",
            "acueducto": "acueducto",
            "simmtrafficdata-clean": "acueducto",
            "simmtrafficdata-clean.csv": "acueducto",
            "energia": "energia",
            "energía": "energia",
            "velocidad-e-intensidad-vehicular-en-medellin-clean": "energia",
            "velocidad-e-intensidad-vehicular-en-medellin-clean.csv": "energia",
        }
        return aliases.get(normalized, normalized)

    def _get_movilidad_file(self, dataset: str) -> Path:
        key = self._normalize_movilidad_dataset(dataset)
        mapping = {
            "gas": "Aforos_Vehiculares_clean.csv",
            "acueducto": "simmtrafficdata_clean.csv",
            "energia": "velocidad_e_intensidad_vehicular_en_medellin_clean.csv",
        }
    ...
    def _load_movilidad_rows(
        self,
        dataset: str,
    ) -> tuple[List[Dict[str, Any]], List[int], List[str], List[str], List[str]]:
        csv_path = self._get_movilidad_file(dataset)
    ...
    def get_tarifas_data(
        ...
        rows, years, months, sectors, estratos = self._load_movilidad_rows(dataset)
        key = self._normalize_movilidad_dataset(dataset)
        filtered = [
            row for row in rows
    ...
    def get_tarifas_por_estrato(self, dataset: str, year: Optional[int] = None) -> List[Dict[str, Any]]:
        rows, _, _, _, _ = self._load_movilidad_rows(dataset)
        filtered = [row for row in rows if (year is None or row["year"] == year)]
    ...
    def _load_empresarial_rows(self) -> tuple[List[Dict[str, Any]], List[str], List[int]]:
        ...
        """
        csv_path = self._get_proyecciones_poblacion_file()
        if not csv_path.exists():
            raise FileNotFoundError(f"Dataset no encontrado en {csv_path}")
        ...
        with open(csv_path, 'r', encoding='utf-8-sig', newline='') as f:
            ...
            # Formato nuevo: proyecciones_de_poblacion_medellin_2019_clean.csv
            if "codigoDane_municipio" in fieldnames:
                comuna_columns = sorted(
                    {
                        _pick_first(r, ["codigo"], "")
                        for r in reader
                        if _pick_first(r, ["tipo_division_geografica"], "").lower() == "comuna"
                    }
                )
                ...
                reader = csv.DictReader(f)
                for row in reader:
                    if _pick_first(row, ["tipo_division_geografica"], "").lower() != "comuna":
                        continue

                    anio = 2019
                    years.add(anio)
                    comuna_nombre = _pick_first(row, ["codigo"], "")
                    total = _to_int(_pick_first(row, ["total_2019"], "0"), 0)

                    comunas: Dict[str, int] = {comuna: 0 for comuna in comuna_columns}
                    if comuna_nombre:
                        comunas[comuna_nombre] = total

                    rows.append(
                        {
                            "anio": anio,
                            "ciiu": _pick_first(row, ["grupo_edad"], "N/A"),
                            "descripcion": _pick_first(row, ["tipo_division_geografica"], "Comuna"),
                            "total_empresas": total,
                            "top_comuna": comuna_nombre or None,
                            "top_comuna_total": total,
                            "comunas": comunas,
                        }
                    )
    ...
    def _load_estratificacion_rows(self) -> tuple[List[Dict[str, Any]], List[str], List[int], List[str]]:
        ...
        with open(csv_path, 'r', encoding='utf-8-sig', newline='') as f:
            ...
            # Formato nuevo: Pasajeros_movilizados_clean.csv
            if "ANO" in fieldnames:
                line_columns = [name for name in fieldnames if name.endswith("_PAX_MOV")]
                for row in reader:
                    periodo = _pick_first(row, ["SEM_ANO", "ANO_MES", "ANO"], "")
                    for col in line_columns:
                        servicio = col.replace("_PAX_MOV", "")
                        suscriptores = _to_int(_pick_first(row, [col], "0"), 0)
                        if suscriptores <= 0:
                            continue
    ...
    def _load_negocios_medellin_rows(
        ...
        csv_path = self._get_aforos_vehiculares_file()
        if not csv_path.exists():
            raise FileNotFoundError(f"Dataset no encontrado en {csv_path}")
        ...
        with open(csv_path, 'r', encoding='utf-8-sig', newline='') as f:
            ...
            # Formato nuevo: Aforos_Vehiculares_clean.csv
            if "NODO" in fieldnames:
                for raw in reader:
                    comuna = _pick_first(raw, ["COMUNA", "NOMBRE_COMUNA"], "")
                    barrio = _pick_first(raw, ["INTERSECCIÓN", "VÍA_PRINCIPAL"], "")
                    tipo_negocio = _pick_first(raw, ["MOVIMIENTO", "ACCESO"], "")
                    categoria = _pick_first(raw, ["TIPO_DE_DIA", "MES"], "")
                    fecha = _pick_first(raw, ["FECHA", "FECHA_HORA", "DIA/MES/AÑO"], "")

                    if _is_blank_or_na(comuna) or _is_blank_or_na(barrio):
                        continue

                    row = {
                        "id": _to_int(_pick_first(raw, ["NODO", "CODIGO"], "0"), 0),
                        "comuna": comuna,
                        "barrio": barrio,
                        "tipo_negocio": "" if _is_blank_or_na(tipo_negocio) else tipo_negocio,
                        "categoria": "" if _is_blank_or_na(categoria) else categoria,
                        "cantidad_actual": _to_int(
                            _pick_first(raw, ["VOLUMEN_TOTAL_HORA", "VEHICULO_EQUIVALENTE", "AUTOS"], "0"),
                            0,
                        ),
    ...
    def get_criminalidad_data(self) -> List[Dict[str, Any]]:
        ...
        """
        csv_path = self.datasets_path / "simmtrafficdata_clean.csv"
        ...
        try:
            ...
            with open(csv_path, 'r', encoding='utf-8-sig', newline='') as f:
                ...
                for row in reader:
                    # Soporta variaciones para dataset histórico y nuevo
                    nombre = _pick_first(row, ["Nombre", "NOMBRE", "Comuna", "CORREDOR", "DISPOSITIVO", "SENTIDO"])
                    total_casos_raw = _pick_first(
                        row,
                        ["[TotalCasos]", "([TotalCasos])", "TotalCasos", "TOTAL_CASOS", "REGISTROS", "INTENSIDAD"],
                        "0"
                    )
                    tasa_raw = _pick_first(
                        row,
                        ["TasaCriminalidad", "Tasa Criminalidad", "tasa_criminalidad", "VELOCIDAD", "OCUPACION"],
                        "0"
                    )
