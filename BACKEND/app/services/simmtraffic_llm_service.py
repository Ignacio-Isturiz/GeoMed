...
_PROYECCIONES_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "datasets", "proyecciones_de_poblacion_medellin_2019_clean.csv"
)


def _load_simmtraffic_rows() -> List[Dict]:
    """Carga el CSV y retorna lista de dicts con nombre, tasa y casos."""
    rows = []
    try:
        with open(_SIMMTRAFFIC_PATH, encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                tasa_raw = row.get("TasaCriminalidad") or row.get("VELOCIDAD") or row.get("OCUPACION") or "0"
                casos_raw = row.get("([TotalCasos])") or row.get("TotalCasos") or row.get("REGISTROS") or row.get("INTENSIDAD") or "0"
                rows.append({
                    "nombre": str(nombre).strip(),
                    "tasa": float(str(tasa_raw).strip().replace(".", "").replace(",", ".") or 0),
                    "casos": int(float(str(casos_raw).strip().replace(".", "").replace(",", "") or 0)),
                })
...
def _load_proyecciones_rows() -> List[Dict]:
    rows: List[Dict] = []
    try:
        with open(_PROYECCIONES_PATH, encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                # Normalize and parse
                try:
                    estrato = (row.get("estrato") or row.get("grupo_edad") or "").strip()
                    servicio = (row.get("servicio") or row.get("tipo_division_geografica") or "").strip()
                    cobertura = (row.get("cobertura") or "").strip()
                    suscriptores = (row.get("suscriptores") or row.get("total_2019") or "").strip()
                    rows.append({
                        "servicio": servicio,
                        "estrato": estrato,
                        "cobertura": cobertura,
