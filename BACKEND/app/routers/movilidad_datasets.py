...
from app.services.movilidad_datasets_service import DatasetsService
...
@router.get(
    ...
    description="Devuelve estadÃ­sticas agregadas de criminalidad"
)
async def get_criminalidad_summary():
    """
    Obtiene un resumen estadÃ­stico de los datos de criminalidad.
    ...
    Returns:
        ...
        - tasa_maxima: MÃ¡xima tasa de criminalidad
        - tasa_minima: MÃ­nima tasa de criminalidad
        - comuna_mas_afectada: Comuna con mÃ¡s criminalidad
    """
    try:
        service = DatasetsService()
...
@router.get(
    "/empresarial/years",
    status_code=status.HTTP_200_OK,
    summary="Obtener aÃ±os disponibles del dataset empresarial"
)
async def get_empresarial_years():
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error al obtener aÃ±os del dataset empresarial"
        )
