...
const SUGERENCIAS_SEG = [
  'Â¿CuÃ¡l es el barrio mÃ¡s seguro para vivir?',
  'Quiero ir a trotar por La Candelaria, Â¿es seguro?',
  'Compara la seguridad entre Laureles y Buenos Aires',
  'Â¿CuÃ¡les son las zonas mÃ¡s peligrosas?',
];

/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   CHATBOT SEGURIDAD â€” solo chat, sin cards extra
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
function ChatSeguridad() {
  const [messages, setMessages] = useState([{
    role:'bot',
    text:'Â¡Hola! Soy el GuardiÃ¡n, tu asistente de seguridad de MedellÃ­n. PregÃºntame sobre cualquier barrio o zona.',
  }]);
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        <div>
          ...
          <div className="db-card-subtitle">Fuentes verificadas Â· MedellÃ­n</div>
        </div>
      ...
      <div style={{ overflowY:'auto', flex:1 }}>
        {loading ? (
          ...
          <a key={i} href={art.url} target="_blank" rel="noreferrer" className="db-news-item">
            <div className="db-news-source">{art.source || 'MedellÃ­n'}</div>
      ...
      {summary && (
        <div className="db-stat-row db-stat-row--4">
          {[
            ...
            {label:'MÃ¡s afectada', value:summary.comuna_mas_afectada, small:true},
          ].map((s,i) => (
            ...
            <tr>
              ...
              <th style={{textAlign:'right'}}>
                <button type="button" className="db-th-sort-btn right" onClick={() => toggle('tasa_criminalidad')} aria-label="Ordenar por tasa de criminalidad">
                  Tasa /100k {sort.key==='tasa_criminalidad'?(sort.dir==='asc'?'â†‘':'â†“'):''}
                </button>
              </th>
            </tr>
          </thead>
...
function AnalizadorWidget() {
  ...
  const addFiles = (files) => {
    ...
    if (!valid.length) { setError('Solo se aceptan imÃ¡genes JPG, PNG, WEBP.'); return; }
    setError(null); setResult(null);
    setImages(prev => {
      const next = [...prev, ...valid.map(f => ({ file:f, preview:URL.createObjectURL(f) }))];
      if(next.length > 6) { setError('MÃ¡ximo 6 imÃ¡genes.'); return prev; }
      return next;
    });
  };
    ...
    <div style={{display:'flex',flexDirection:'column',gap:10,overflowY:'auto'}}>
      ...
      {result.prediccion && (
        <div className="db-result-block pred">
          <div className="db-result-label">PredicciÃ³n prÃ³xima factura</div>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      ...
      >
        ...
        <div className="db-drop-icon">ðŸ“„</div>
        <div className="db-drop-text">Arrastra o <span className="db-drop-link">haz clic para agregar</span> fotos de tu factura</div>
        <div className="db-drop-hint">EnergÃ­a Â· Acueducto Â· Gas Â· JPG, PNG, WEBP Â· MÃ¡x. 6</div>
      </div>
      {images.length > 0 && (
        <div className="db-img-preview-grid">
          {images.map((img,i) => (
            <div key={i} className="db-img-preview">
              <img src={img.preview} alt={`PÃ¡g ${i+1}`}/>
              <button className="db-img-remove" onClick={()=>setImages(p=>p.filter((_,j)=>j!==i))}>âœ•</button>
            </div>
          ))}
        </div>
      )}
...
export default function CiudadanoDashboard() {
  ...
  if(loading) return <div className="db-loading"><div className="db-spinner"/>Cargando tu ciudadâ€¦</div>;
  if(!user) return null;
  ...
  const META = {
    ...
    seguridad: { accent:'',      title:'Seguridad',        subtitle:'Chatbot GuardiÃ¡n y criminalidad por zona' },
    servicios: { accent:'',      title:'Servicios PÃºblicos',subtitle:'AnÃ¡lisis de facturas EPM con inteligencia artificial' },
    noticias:  { accent:'',      title:'Noticias',         subtitle:'Actualidad de MedellÃ­n filtrada por categorÃ­a' },
  };
  const m = META[mod];

  /* â•â• Bloque Ciudad Inteligente (compartido) â•â• */
  const ciudadInteligente = (
    <div className="db-rc db-card-note" style={{flexShrink:0}}>
      ...
      <div className="db-note-body">
        MECIA integra <b>datos reales</b> de MedellÃ­n: seguridad, servicios pÃºblicos y noticias en tiempo real.
      </div>
      <div className="db-note-footer">
        <span className="db-note-time">MedellÃ­n Â· Valle de AburrÃ¡</span>
        <div className="db-note-badge"><span className="ck">âœ“</span> Conectado</div>
      </div>
    </div>
  );

  /* â•â• Bloque seguridad resumen (con tooltip) â•â• */
  const seguridadResumen = crimiSummary ? (
    <div className="db-rc" style={{flexShrink:0}}>
      <div className="db-card-header" style={{marginBottom:10}}>
        <span className="db-card-title">Seguridad Â· Resumen</span>
      </div>
      <div className="db-mini-stats-grid">
        {[
          ...
          {label:'+ afectada',    value:crimiSummary.comuna_mas_afectada, small:true, tooltip:'Comuna con mayor nÃºmero de casos reportados'},
        ].map((s,i)=>(
  ...
  /* â•â• COL-R dinÃ¡mico por mÃ³dulo â•â• */
  const rightColByMod = {
    inicio: (
      <>
        {ciudadInteligente}
        ...
        <div className="db-rc" style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          ...
          <div style={{overflowY:'auto',flex:1}}>
            {topNoticias.length === 0 ? (
              ...
              >
                <div style={{fontSize:10,fontWeight:700,letterSpacing:'.04em',textTransform:'uppercase',color:'var(--accent)',marginBottom:3}}>{art.source||'MedellÃ­n'}</div>
      ...
      <>
        ...
        <div className="db-rc" style={{flex:1,display:'flex',flexDirection:'column',minHeight:0}}>
          <div className="db-card-header" style={{marginBottom:10,flexShrink:0}}>
            <span className="db-card-title">CÃ³mo usar el analizador</span>
          </div>
          <div style={{fontSize:13,color:'var(--text-mid)',lineHeight:1.7,display:'flex',flexDirection:'column',gap:10,flex:1}}>
            {[
              ...
              {step:'2', txt:'Arrastra o sube hasta 6 imÃ¡genes (JPG, PNG, WEBP)'},
              {step:'3', txt:'La IA analiza el consumo y detecta anomalÃ­as'},
              {step:'4', txt:'Recibe recomendaciones de ahorro personalizadas'},
              {step:'5', txt:'ObtÃ©n predicciÃ³n de tu prÃ³xima factura'},
            ].map(({step,txt})=>(
        ...
        <div className="db-rc" style={{flex:1,display:'flex',flexDirection:'column',minHeight:0}}>
          <div className="db-card-header" style={{marginBottom:10,flexShrink:0}}>
            <span className="db-card-title">Tarifas EPM Â· Referencia</span>
          </div>
          <div className="db-tarifas-grid" style={{flex:1,minHeight:0}}>
            {[
              {servicio:'EnergÃ­a', icon:'âš¡', color:'#d97706', estratos:[
                {e:'E1â€“2', rango:'$45kâ€“$90k'},
                {e:'E3â€“4', rango:'$90kâ€“$140k'},
                {e:'E5â€“6', rango:'$140kâ€“$200k'},
              ]},
              {servicio:'Acueducto', icon:'ðŸ’§', color:'#60a5fa', estratos:[
                {e:'E1â€“2', rango:'$20kâ€“$40k'},
                {e:'E3â€“4', rango:'$40kâ€“$60k'},
                {e:'E5â€“6', rango:'$60kâ€“$80k'},
              ]},
              {servicio:'Gas', icon:'ðŸ”¥', color:'#f87171', estratos:[
                {e:'E1â€“2', rango:'$15kâ€“$25k'},
                {e:'E3â€“4', rango:'$25kâ€“$40k'},
                {e:'E5â€“6', rango:'$40kâ€“$60k'},
              ]},
          ...
          <div style={{fontSize:10,color:'var(--text-dim)',borderTop:'1px solid var(--sep)',paddingTop:7,marginTop:8,lineHeight:1.5,flexShrink:0}}>
            Valores estimados por estrato Â· sube tu factura para anÃ¡lisis exacto.
          </div>
        </div>
      </>
    ),
    noticias: (
      <>
        ...
        <div className="db-rc" style={{flex:1,display:'flex',flexDirection:'column',minHeight:0}}>
          <div className="db-card-header" style={{marginBottom:10,flexShrink:0}}>
            <span className="db-card-title">CategorÃ­as disponibles</span>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:7,flex:1}}>
            {[
              {v:'general',        label:'General',        color:'#00C896', desc:'Noticias generales de la ciudad'},
              {v:'seguridad',      label:'Seguridad',      color:'#f87171', desc:'Orden pÃºblico y convivencia'},
              {v:'emprendimiento', label:'Emprendimiento', color:'#60a5fa', desc:'EconomÃ­a, startups y negocios'},
              {v:'movilidad',      label:'Movilidad',      color:'#a78bfa', desc:'Metro, tranvÃ­a y transporte'},
              {v:'salud',          label:'Salud',          color:'#34d399', desc:'Salud pÃºblica y bienestar'},
              {v:'economia',       label:'EconomÃ­a',       color:'#d97706', desc:'Indicadores y mercado laboral'},
  ...
  /* â•â• COL-L: contenido principal segÃºn mÃ³dulo â•â• */

  // â”€â”€ INICIO: overview con datos â”€â”€
  const inicioLeft = (
    <>
      ...
      {crimiSummary && (
        <div className="db-stat-row db-stat-row--4" style={{flexShrink:0}}>
          {[
            {label:'Comunas con datos',  value:crimiSummary.total_comunas, tooltip:'NÃºmero de comunas de MedellÃ­n con datos de criminalidad disponibles'},
            ...
            {label:'Tasa por 100k hab.', value:Number(crimiSummary.tasa_promedio).toFixed(1), tooltip:'Promedio de casos por cada 100.000 habitantes en MedellÃ­n'},
            {label:'Comuna mÃ¡s afectada',value:crimiSummary.comuna_mas_afectada, small:true, tooltip:'La comuna con mÃ¡s casos de criminalidad registrados'},
          ].map((s,i)=>(
            <div key={i} className="db-stat-item" data-tooltip={s.tooltip}>
              <div className="db-stat-label">{s.label}</div>
      ...
      <div className="db-inicio-split" style={{flex:1,overflow:'hidden',minHeight:0}}>
        ...
        <div className="db-card" style={{display:'flex',flexDirection:'column',gap:14,overflow:'hidden'}}>
          <div className="db-card-header" style={{marginBottom:0}}>
            <div>
              <div className="db-card-title">GuardiÃ¡n Â· Asistente de Seguridad</div>
              <div className="db-card-subtitle">Consulta sobre barrios, zonas y criminalidad en MedellÃ­n</div>
            </div>
          ...
          <div style={{fontSize:13,color:'var(--text-mid)',lineHeight:1.7}}>
            El GuardiÃ¡n analiza datos reales de criminalidad por comunas y te ayuda a tomar decisiones informadas sobre seguridad en la ciudad.
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:8,flex:1}}>
            <div style={{fontSize:11,fontWeight:700,letterSpacing:'.07em',textTransform:'uppercase',color:'var(--text-dim)'}}>
              Puedes preguntarle sobreâ€¦
            </div>
            {SUGERENCIAS_SEG.slice(0,3).map((s,i)=>(
              <div key={i} style={{
                ...
                <span style={{color:'var(--accent)',flexShrink:0}}>â€º</span>{s}
              </div>
            ))}
          </div>
        ...
        <div className="db-card" style={{display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div className="db-card-header" style={{marginBottom:10,flexShrink:0}}>
            <div>
              <div className="db-card-title">Explorar MedellÃ­n</div>
              <div className="db-card-subtitle">Zonas recomendadas para visitar</div>
            </div>
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:8,flex:1,minHeight:0}}>
            {[
              { zona:'El Poblado',  desc:'Zona rosa, restaurantes y comercio seguro',   tag:'TurÃ­stico',  color:'#00C896' },
              { zona:'Laureles',    desc:'Parques, barrio residencial y gastronomÃ­a',    tag:'Familiar',   color:'#3B82F6' },
              { zona:'Envigado',    desc:'Tranquilo, fÃ¡cil acceso al metro',             tag:'Residencial',color:'#8B5CF6' },
              { zona:'El Centro',   desc:'Historia, museos, Parque BerrÃ­o y el Metro',  tag:'Cultural',   color:'#F59E0B' },
        ...
        {[
          ...
          >
            ...
            <div>
              ...
              <div style={{fontSize:11,color:'var(--accent)',fontWeight:600,marginTop:6}}>Abrir mÃ³dulo â†’</div>
            </div>
          </button>
        ))}
      </div>
  ...
  // â”€â”€ SEGURIDAD: solo chatbot y tabla, sin cards extra â”€â”€
  const seguridadLeft = (
    <div className="db-card" style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      <TabBar
        tabs={[
          {id:'chatbot',      label:'Chatbot GuardiÃ¡n'},
  ...
  // â”€â”€ SERVICIOS: solo el analizador â”€â”€
  const serviciosLeft = (
    <div className="db-card" style={{flex:1,overflowY:'auto'}}>
      <div className="db-card-header">
        <div>
          <div className="db-card-title">Analizador de Factura EPM</div>
          <div className="db-card-subtitle">Sube fotos de tu factura Â· Recomendaciones + predicciÃ³n con GPT-4o Vision</div>
        </div>
      </div>
  ...
  const noticiasLeft = (
    <div className="db-card" style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',padding:0}}>
      <div style={{padding:'18px 22px 14px',flexShrink:0}}>
        <div className="db-card-title">Noticias de MedellÃ­n</div>
        <div className="db-card-subtitle">Fuentes verificadas Â· Filtradas por categorÃ­a</div>
      </div>
